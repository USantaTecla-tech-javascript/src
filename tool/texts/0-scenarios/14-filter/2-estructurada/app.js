...
let COUNTRY_NAMES = COUNTRIES.map(country => country.name);
let COUNTRY_REGIONS = COUNTRIES.map(country => country.region);
let COUNTRY_POPULATIONS = COUNTRIES.map(country => country.population);
let COUNTRY_SUBREGIONS = COUNTRIES.map(country => country.subregion);
let COUNTRY_AREAS = COUNTRIES.map(country => country.area);

let region = `Asia`;
let population = 10000000;
let answer = ``;
for (let i = 0; i < COUNTRY_NAMES.length; i++) {
  if (COUNTRY_REGIONS[i] === region && COUNTRY_POPULATIONS[i] >= population) {
    answer += `${answer === `` ? `` : `, `}${COUNTRY_NAMES[i]}`;
  }
}
...
answer = ``;
for (let i = 0; i < COUNTRY_NAMES.length; i++) {
  if (COUNTRY_SUBREGIONS[i] === subregion && COUNTRY_AREAS[i] >= area) {
    answer += `${answer === `` ? `` : `, `}${COUNTRY_NAMES[i]}`;
  }
}
