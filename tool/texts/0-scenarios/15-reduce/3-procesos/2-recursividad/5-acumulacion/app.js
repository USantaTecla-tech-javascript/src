...
console.writeln(`¿Cuál es la lista de países?: ${getList(getCountries())}`);
console.writeln(`¿Cuál es la tabla de países?: ${getHTMLTable(getCountries())}`);

function getList([country, ...countries], index, length) {
  if (index === undefined) {
    return getList(countries, 0, [country, ...countries].length);
  }
  if (country === undefined) {
    return ``;
  }
  return `${index === 0 ? `` : `${index < length - 1 ? `,` : ` y`} `}${country}` +
    getList(countries, index + 1, length);
}

function getHTMLTable(countries) {
  return `<html><body><table>\n${getRows(countries)}</table></body></html>\n`;
  
  function getRows([country, ...countries]){
    if (country === undefined) {
      return ``;
    }
    return  `  <tr><td>${country}</td></tr>\n` + 
    getRows(countries);
  }   
}
