...
console.writeln(`¿Cuál es la lista de países?: ${
  reduce(getCountries(), getItem, ``)}`);
console.writeln(`¿Cuál es la tabla de países?: ${
  reduce(getCountries(), getRow, `<html><body><table>\n`)}</table></body></html>\n`);

function reduce(countries, reducer, initial) {
  let list = initial;
  for (let i = 0; i < countries.length; i++) {
    list += reducer(countries[i], i, countries);
  }
  return list;
}

function getItem(country, index, countries) {
  return `${index === 0 ? `` : `${index < countries.length - 1 ? `,` : ` y`} `}${country}`
}
