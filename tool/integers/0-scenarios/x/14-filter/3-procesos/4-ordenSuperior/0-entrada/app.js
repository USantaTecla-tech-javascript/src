...
writelnNames(`¿Cuál es el nombre de los paises de ${region} con población superior o igual a ${population}?:`,
  filter(getNameRegionPopulationCountries(), searchByRegionPopulation, [region, population]));
...
writelnNames(`¿Cuál es el nombre de los paises de ${subregion} con area superior o igual a ${area}?:`,
  filter(getNameSubregionAreaCountries(), searchBySubregionArea, [subregion, area]));
...
function filter(countries, search, args) {
  ...
  for (let i = 0; i < countries[NAME].length; i++) {
    let country = [];
    for(let j= 1; countries.length; j++){
      country[j] = countries[i];
    }
    if (search(country, args)) {
...
function getNameRegionPopulationCountries() {
  return [
    getCountries().map(
...
function getNameSubregionAreaCountries() {
  return [
    getCountries().map(
      country => country.name),
    getCountries().map(
