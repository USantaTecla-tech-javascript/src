...
console.writeln(`¿Cuál es la población total?: ${sum}`);

let list = ``;
for (let i = 0; i < COUNTRIES.length; i++) {
  list += `${i === 0? `` : `, `}${COUNTRIES[i].alpha2Code}`
}
console.writeln(`¿Cuál es la lista de "alpha2code"?: ${list}`);

let iMax = 0;
for (let i = 1; i < COUNTRIES.length; i++) {
  if (COUNTRIES[iMax].area < COUNTRIES[i].area) {
    iMax = i;
  }
}
console.writeln(`¿Cuál es el área del mayor país?: ${COUNTRIES[iMax].area}`);
