...
console.writeln(`¿Cuál es la población total?: ${reduce(getCountries(), generateGetSumPopulation(), 0)}`);
console.writeln(`¿Cuál es la lista de "alpha2code"?: ${reduce(getCountries(), generateGetListAlpha2Codes(), `Alpha2Codes: `)}`);
console.writeln(`¿Cuál es el área del mayor país?: ${reduce(getCountries(), generateGetMaxArea(), 0)}`);

function reduce(countries, reducer, initial) {
  let acum = initial;
  if (countries === []) {
    return acum;
  }
  for (let i = 0; i < countries.length; i++) {
    acum = reducer(acum, countries[i], i);
  }
  return acum;
}

function generateGetSumPopulation() {
  function getSumPopulation(population, country) {
    return typeof country.population === "number" ? population + country.population : 0;
  };

  return getSumPopulation;
}

function generateGetListAlpha2Codes() {
  function getAlpha2CodeList(list, country, index) {
    return `${list}${index === 0 ? `` : `, `}${country.alpha2Code}`
  }

  return getAlpha2CodeList;
}

function generateGetMaxArea(){
  function getMaxArea(max, country) {
    return max > country.area ? max : country.area;
  };
