...
function generateGetSumPopulation() {
  return (population, country) => typeof country.population === "number" ? population + country.population : 0;
}

function generateGetListAlpha2Codes() {
  return (list, country, index) => `${list}${index === 0 ? `` : `, `}${country.alpha2Code}`;
}

function generateGetMaxArea(){
  return (max, country) => max > country.area ? max : country.area;
}
