...
console.writeln(`¿Cuál es la población total?: ${getSumPopulatioin(getCountries())}`);
console.writeln(`¿Cuál es la lista de "alpha2code"?: ${getListAlpha2Code(getCountries())}`);
console.writeln(`¿Cuál es el área del mayor país?: ${getMaxArea(getCountries())}`);

function getSumPopulatioin(countries) {
  let result = 0;
...
function getListAlpha2Code(countries) {
  let result = ``;
  for (let i = 0; i < countries.length; i++) {
    result += `${i === 0 ? `` : `, `}${countries[i].alpha2Code}`;
  }
  return result;
}

function getMaxArea(countries) {
  let iResult = 0;
  for (let i = 1; i < countries.length; i++) {
    if (countries[iResult].area < countries[i].area) {
      iResult = i;
    }
  }
  return countries[iResult].area
}
