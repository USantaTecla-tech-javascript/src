...
console.writeln(`¿Cuál es la población total?: ${reduce(getCountries(),
  (population, country) => typeof country.population === "number" ? population + country.population : 0, 0)}`);
console.writeln(`¿Cuál es la lista de "alpha2code"?: ${reduce(getCountries(),
  (list, country, index) => `${list}${index === 0 ? `` : `, `}${country.alpha2Code}`, `Alpha2Codes: `)}`);
console.writeln(`¿Cuál es el área del mayor país?: ${reduce(getCountries(),
  (max, country) => max > country.area ? max : country.area, 0)}`);

function reduce([head, ...tail], reducer, initial) {
  return head === undefined ?
      initial
    :
      reduce(tail, reducer, reducer(initial, head));
}
