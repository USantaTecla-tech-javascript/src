...
writelnNames(`¿Cuál es el nombre de los paises de ${region} con población superior o igual a ${population}?:`,
  filter(getCountries(), (country) => country.region === region && country.population >= population));
...
writelnNames(`¿Cuál es el nombre de los paises de ${subregion} con población superior o igual a ${area}?:`,
  filter(getCountries(), (country) => country.subregion === subregion && country.area >= area));
...
function filter([head, ...tail], search) {
  return head === undefined ?
    []
    :
    search(head) ?
      [head, ...filter(tail, search)]
      :
      filter(tail, search);
}
